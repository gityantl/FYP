...
for cas_const_de in np.arange (cas_const_de,0,20.45):
    ...
    print(de_data, type(de_data), de_data.shape)
    ...
    de_data.to_csv(r'D://trajectory/descent.csv', index=False)
    de_data: numpy array, np.save(r'D://trajectory/descent.npy', de_data)


plt.figure(1)
plt.plot(de_data["t"], de_data["h"] / aero.ft, label="%d/%.2f" % (de_data["cas_const_de"]))
plt.legend()
plt.xlim()
plt.show()
